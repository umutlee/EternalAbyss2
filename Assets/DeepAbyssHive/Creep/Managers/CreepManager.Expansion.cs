...
namespace DeepAbyssHive.Creep.Managers
{
    ...
    public partial class CreepManager
    {
        ...
        private float GetExpansionPriority(Vector2Int position)
        {
            ...
            // 靠近建筑的位置优先级更高
            var nearbyBuildings = GetNearbyBuildings(position, 3f);
            priority += nearbyBuildings.Count * 10f;
            
            // 靠近资源点的位置优先级更高
            var nearbyResources = GetNearbyResources(position, 2f);
            priority += nearbyResources.Count * 5f;
        ...
        public bool ExpandAroundBuilding(BuildingData building, float radius = 2f)
        {
            ...
            var targetPositions = GetPositionsInRadius(building.Position, radius);
            var validPositions = new List<Vector2Int>();
            
            foreach (var pos in targetPositions)
            {
                if (CanExpandToPosition(pos))
                {
        ...
        private bool TryExpandToPosition(Vector2Int position, bool consumeResources = true)
        {
            ...
            // 触发扩张事件
            OnCreepExpanded?.Invoke(position, creepTile);
