...
namespace DeepAbyssHive.Core.Services
{
    ...
    public class ServiceRegistrar : MonoBehaviour
    {
        ...
        {
            // 这里暂时创建空的服务实现，后续会替换为真实的Manager服务
            // serviceManager.RegisterService<ITerrainQueryService>(new TerrainQueryService());
            // serviceManager.RegisterService<ITerrainModificationService>(new TerrainModificationService());
            
            Debug.Log("[ServiceRegistrar] 地形服务注册完成（待实现）");
        }
