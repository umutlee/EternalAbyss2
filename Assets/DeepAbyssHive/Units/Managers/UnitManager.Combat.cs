...
namespace DeepAbyssHive.Units.Managers
{
    ...
    public partial class UnitManager
    {
        ...
        public void AttackTarget(int unitId, int targetId)
        {
            ...
            // 更新单位状态
            hotData.CurrentState = UnitState.Attacking;
            hotData.TargetUnitId = targetId;
        ...
        private void PerformAttack(int attackerId, int targetId)
        {
            ...
            if (targetHotData.Health <= 0)
            {
                ...
                targetHotData.CurrentState = UnitState.Dead;
        ...
        {
            // 检查目标是否存在
            if (!_unitHotData.TryGetValue(hotData.TargetUnitId, out UnitHotData targetHotData))
            {
                // 目标不存在，回到空闲状态
            ...
            // 检查攻击冷却
            if (hotData.ActionTimer <= 0 && hotData.Velocity == Vector3.zero)
            {
                // 执行攻击
                PerformAttack(unitId, hotData.TargetUnitId);
                
                // 重置攻击冷却
                hotData.ActionTimer = 1f / coldData.BaseAttributes.AttackSpeed;
            }
        }
    }
}
